...
class ProbabilisticLSystem extends BaseLSystem {
 ...
 // Production rules
  ...
  public ProbabilisticLSystem(String axiom, HashMap<Character, String[]> rules, 
    float moveDistance, float rotateAngle, float scaleFactor, float angleVariation, float distanceVariation) {            
  ...
  public void iterate() {
    ...
    for (int i = 0; i < current.length(); i++) {
      ...
      if (rules.containsKey(c)) {
        String[] newRules = rules.get(c);
        int ruleIndex = int(random(newRules.length));
        newS = newRules[ruleIndex];
      }
        
      this.currentIterationBuffer.append(newS);
    }
  ...
  public void drawLSystem(Turtle t) {
    ...
    float dist = this.moveDistance + map(noise(time), 0, 1, -this.distanceVariation, this.distanceVariation);
    float angle = this.rotateAngle + map(noise(time + 10), 0, 1, -this.angleVariation, this.angleVariation);
    this.time += 1;
